...
export default function ListingsClient({
  ...
  // The first page is rendered on the server so crawlers see the listings, so
  // refetching it on mount would only duplicate that request.
  const serverRendered = useRef(initialListings.length > 0);
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (data?.user?.approved && !localStorage.getItem("approved_notified")) {
